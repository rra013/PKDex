...
actor PokeSyncManager {
    ...
    /// Species allowed in the currently-active Champions regulation, sourced
    /// from `champions-<id>.json` via `ChampionsRegulation`. This used to be a
    /// hand-curated `Set<String>` here AND in `champsDex.swift`, and both had
    /// drifted relative to the JSON (Mon Index showed Ursaluna but not
    /// Scovillain). One source of truth lives in the bundled regulation JSON.
    static var championsRoster: Set<String> {
